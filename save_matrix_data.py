		...
		if len(matrices) == 1:
			...
			if label in labelnames:
				if not (len(labelnames[label]) == len(matrices)):
					to_string  = '* Error: Number of matrices for '+str(label)+' does not match expected from labelnames.\n'
					to_string += '         No of matrices: '+str(len(matrices))+'\n'
					to_string += '         Number of matrices expected (from labelnames): '+str(len(labelnames[label]))+'\n'
					to_string += '         Expected matrices (from labelnames): '+str(labelnames[label])
					print(to_string)
...
def convert_into_matrix(array, matrix_information, label):
	...
	if not (len(matrices) == no_of_matrices):
		to_string  = '* Error: Number of matrices for '+str(label)+' does not match expected.\n'
		to_string += '         No of matrices: '+str(len(matrices))+'\n'
		to_string += '         Number of matrices expected: '+str(no_of_matrices)+'\n'
		to_string += '         matrix_information: '+str(matrix_information)
		print(to_string)
		#import pdb; pdb.set_trace()
