...
from supplementary_methods import save_data, append_data, get_infotypes, get_datatype, entrylengths, get_filename
...
def extract_data(data_filename, matrix_elements_foldername):
	...
	with open(matrix_elements_foldername+'/'+data_filename, "r") as outfile:
		for line_number, line in enumerate(outfile, start=1):
			...
			# Third, get the name of the matrix being extracted. 
			label = None if ('Label' not in data) else get_filename(data['Label'])

			# Fourth, obtain the data from the line and import it into the data dictionary. 
			is_matrix_data, infotype = determine_matrix_data(line)
			try:
				if is_matrix_data:
					extract_matrix_data(line, infotype, data, line_number, label)
				else:
					extract_non_matrix_data(split_line, data, line_number, label)
			except Exception as exception:
				print(f'* Warning: There was a problem with extracting line {line_number} of matrix_element_output.txt (Matrix name: {label}). Had to move on. The CSV file for {label} may be incomplete.')

	# Fifth, save the final compoent in data if this has not already been saved. 
	if not (data == {}):
		label, filepaths = save_matrix_data(data, matrix_elements_foldername)
		if label is not None:
			labels.append(label)
	...
	# Sixth, return all the labels
	return labels, all_filepaths

# ============================================================================================================

def extract_matrix_data(line, infotype_with_equals, data, line_number, label):
	"""
	...
	for index in range(total_index):
		...
		if infotype not in data:
			data[infotype]  = save_data(value, get_datatype(infotype), line_number, label)
		else:
			data[infotype] += append_data(value, get_datatype(infotype), line_number, label)

# ============================================================================================================

def extract_non_matrix_data(split_line, data, line_number, label):
	"""
	...
	for values in split_line:
		...
		if values == 'Title':
			...
			if found_label and ('(' in values) or (')' in values):
				...
				if value0 in get_infotypes():
					...
					if working_infotype not in data:
						data[working_infotype]  = save_data(value, get_datatype(working_infotype), line_number, label)
					else:
						data[working_infotype] += append_data(value, get_datatype(working_infotype), line_number, label)

				else:

					# Problem has arisen. 
					raise Exception('Huh? Unexpected problem. Could you please report this in Github (Thanks) : '+str(value0))
			...
			if working_infotype not in data:
				data[working_infotype]  = save_data(values, get_datatype(working_infotype), line_number, label)
			else:
				data[working_infotype] += append_data(values, get_datatype(working_infotype), line_number, label)
