...
Before beginning, you will need to make sure you have installed or have in your ``lmod``:``module`` system the following:

* GCC: The [GNU Compiler Collection](https://gcc.gnu.org/)
* If you are using the ``lmod``:``module`` system, its probably a good idea to load the save version of GCCCore as your GCC. For example, if you load ``GCC/8.2.0``, also load ``GCCcore/8.2.0``.
* [Python3](https://www.python.org/)
  * For the version of python you use, also install ``numpy`` by performing the following in the terminal: ``pip3 install --upgrade --user numpy``

**Note**: If you are using a ``lmod``:``module`` system, look into using [``EasyBuild`` to install these programs](https://docs.easybuild.io/).
...
The start of the ``setup_GauOpen.sh`` script includes ``module load`` commands that will load the Fortran compiler and Python. 
...
If you do need to include these (because you are using ``slurm``, ``PBS``, or ``TORQUE`` for example), you may need to figure out which versions of ``GCC/GCCCore`` and ``Python`` you need to use. For example, for others in my group the above did not work, but the following ``GCC/GCCCore`` and ``Python`` modules did work:
...
``. At the bottom of your file, include the name of the file you want Gaussian to place the matrix data you desire into. **This is the recommended way to do this**. 
...
This is still a new program and files may not be created properly. It is also common that not all the data can be obtained using the ``gauopen_v2`` program due to errors reading the ``dat`` file. Where possible, check that the data make sense to you. If it doesn't, create an issue in the ``Github Issues Page`` for this program and write the problem(s) you have here: https://github.com/geoffreyweal/RGME/issues
...
**Note**: The ```f2py3``` program is apart of ``numpy``, so try using a different version of python and reinstall ``numpy`` via ``pip3``:
...
### When running ```bash setup_GauOpen.sh```, I get the following error --> ``/home/software/apps/python/3.7.3/bin/python3.7: error while loading shared libraries: libpython3.7m.so.1.0: cannot open shared object file: No such file or directory``
...
rm -fr domp2 readmat readmat8 writemat writemat8 *.f *.so *.o *.mat *.dat *.log __pycache__
gfortran -c -g -O0 qcmatrixio.F
gfortran -c -g -O0 qcmatrix.F
gfortran -g -O0 -o readmat readmat.F qcmatrixio.o qcmatrix.o
cat readmat.F qcmatrixio.F qcmatrix.F >readmat8.F
gfortran -fdefault-integer-8 -fdefault-real-8 -DUSE_I8 -g -O0 -o readmat8 readmat8.F
rm -f readmat8.F
gfortran -g -O0 -o writemat writemat.F qcmatrixio.o qcmatrix.o
cat writemat.F qcmatrixio.F qcmatrix.F >writemat8.F
gfortran -fdefault-integer-8 -fdefault-real-8 -DUSE_I8 -g -O0 -o writemat8 writemat8.F
rm -f writemat8.F
cpp qcmatrixio.F qcmatrixio.f
f2py3  -DF2PY_REPORT_ON_ARRAY_COPY=1 -m qcmatrixio -c qcmatrixio.f
/home/software/apps/python/3.7.3/bin/python3.7: error while loading shared libraries: libpython3.7m.so.1.0: cannot open shared object file: No such file or directory
make: *** [qc.make:31: qcmatrixio.cpython-34m.so] Error 127
mv: cannot stat 'qcmatrixio.cpython*.so': No such file or directory
```

If you are using ``Slurm``/``PBS``/``TORQUE``, this seems to be solved by changing the version of python you are using to the one it is listing. In this example, another person was using ``GCCcore/5.4.0``, ``GCC/5.4.0``, and ``python/3.6.8``. However, it seems that this installation using ``bash setup_GauOpen.sh`` wants to use python 3.7. In this case, He loaded python 3.7, which automatically loaded gcc 8.2.0. For this reason, we changed these module to load the following:
...
and this seemed to work. So try to change the version of ``GCC``/``GCCcore``, and ``Python`` and see if this works. Remember to make sure that ``numpy`` has been installed in this version of python:
